...
public:
   ...
   */
    ...
    inline void *get_mem(AccT acc) {
        if (!ih) {
            throw oneapi::mkl::exception("sparse_blas", "get_mem", "Internal error: CusparseScopedContextHandler must be created inside an host_task to get the data from a buffer");
        }
